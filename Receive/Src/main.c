...
/**
  ******************************************************************************
  * @file           : main.c
  * @brief          : ASK Receiver Main Program
  ******************************************************************************
  * @attention
  *
...
int main(void)
{
  ...
  while (1)
  {
    /* Check for new valid frame and feed to GUI */
    SignalProcessor_GetStats(&stats);
    if (stats.frame_ready)
    {
      RxFrame_t frame;
      SignalProcessor_GetFrame(&frame);
      if (frame.crc_ok)
        LCDDisplay_OnFrame(frame.type, frame.payload, frame.len);
    }

    /* Update display + button scan */
    LCDDisplay_Update(&stats);
    LCDDisplay_Process();
    /* USER CODE END WHILE */
